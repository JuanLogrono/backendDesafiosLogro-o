...
passport.use('registro', new LocalStrategy(async (username, password, callback) => {
    try {
        ...
        if (userReg.length > 0) { return callback(new Error('usuario ya registrado')) };
...
passport.use('auth', new LocalStrategy(async (username, password, callback) => {
    ...
    if (!userLog || !bcrypt.compareSync(password, userLog.password)) return callback(new Error("usuario o contraseña invalido"));
